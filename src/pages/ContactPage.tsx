...
import { Mail, MapPin, Send, MessageCircle, CheckCircle2, AlertTriangle, Info } from 'lucide-react';
...
export const ContactPage: React.FC = () => {
  ...
  const [dispatchInfo, setDispatchInfo] = useState<{ emailId?: string; clientConfirmationSent?: boolean } | null>(null);
    ...
    try {
      ...
      if (res.ok && result.success) {
        ...
        setDispatchInfo({
          ...
          clientConfirmationSent: result.clientConfirmationSent,
        });
        setSubmitted(true);
      } else {
        const errorDetail = result.error?.message || (typeof result.error === 'string' ? result.error : 'An unexpected error occurred while dispatching email via Resend.');
        setErrorMessage(errorDetail);
      }
    } catch (err: unknown) {
      console.error('Error submitting form:', err);
      const networkError = err instanceof Error ? err.message : 'Network error';
      setErrorMessage(`Failed to connect to contact service (${networkError}). Please contact us directly via WhatsApp.`);
    } finally {
                ...
                {submitted ? (
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-1">
                      <h3 className="text-2xl font-bold text-white">Proposal Request Dispatched!</h3>
                      <p className="text-[#00d8ff] text-xs font-semibold uppercase tracking-wider">
                        Routed to NextGen AI Architecture Team
                      </p>
                    </div>
                    ...
                    <div className="w-full max-w-md bg-black/40 border border-white/10 rounded-2xl p-4 text-left flex flex-col gap-2.5">
                      ...
                      {submittedEmail && (
                        <div className="flex justify-between items-center text-xs pb-2 border-b border-white/5">
                          <span className="text-white/50">Receipt Dispatched To</span>
                          <span className="text-[#00d8ff] font-mono font-medium">{submittedEmail}</span>
                        </div>
                      )}
                      {dispatchInfo?.emailId && (
                        <div className="flex justify-between items-center text-xs pb-2 border-b border-white/5">
                          <span className="text-white/50">Resend Delivery ID</span>
                          <span className="text-white/60 font-mono text-[10px] truncate max-w-[180px]">{dispatchInfo.emailId}</span>
                        </div>
                      )}
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-white/50">Expected Turnaround</span>
                        <span className="text-[#25D366] font-semibold">2 to 4 Hours</span>
                      </div>
                    </div>

                    {/* Important Gmail Spam Tip */}
                    <div className="w-full max-w-md p-3.5 rounded-xl bg-[#00d8ff]/5 border border-[#00d8ff]/20 text-left flex items-start gap-3">
                      <Info className="w-4 h-4 text-[#00d8ff] shrink-0 mt-0.5" />
                      <div className="text-xs text-white/70 leading-relaxed">
                        <span className="text-white font-semibold block mb-0.5">Check Gmail Spam / Updates folder:</span>
                        Because <code className="text-[#00d8ff]">connect@getnextgen.in</code> is a freshly verified domain, Google Mail may initially deposit the confirmation receipt into your <strong className="text-white">Spam</strong> or <strong className="text-white">Updates</strong> folder. If you do not see it within 2 minutes, mark it as <em>"Not Spam"</em>.
                      </div>
                    </div>

                    <div className="flex flex-col sm:flex-row items-center gap-3 w-full max-w-md mt-2">
                      <a
                        href="https://wa.me/917385750187?text=Hi%20NextGen%20AI!%20I%20just%20submitted%20a%20proposal%20request%20on%20your%20website."
                        target="_blank"
                      ...
                      >
                        ...
                        <span>Direct WhatsApp Followup</span>
                      </a>
                      ...
                      <button
                        ...
                        className="w-full sm:w-auto px-5 py-3 rounded-xl text-xs font-semibold uppercase tracking-wider bg-white/10 text-white hover:bg-white/20 transition-colors"
                      >
                        Send Another
                      </button>
                    </div>
                    ...
                    {errorMessage && (
                      <div className="p-4 rounded-2xl bg-red-500/10 border border-red-500/30 text-red-300 text-xs sm:text-sm flex flex-col gap-2">
                        <div className="flex items-center gap-2 font-semibold text-red-200">
                          ...
                          <span>Dispatch Alert / Delivery Issue</span>
                        </div>
                        <p className="font-mono text-xs bg-black/40 p-2.5 rounded-lg border border-red-500/20 break-words text-red-200">
                          {errorMessage}
                        </p>
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-2 border-t border-red-500/20 text-white/70 text-xs">
                          <span>Need instant consultation? Contact us on WhatsApp:</span>
                          <a
                            href="https://wa.me/917385750187?text=Hi%20NextGen%20AI!%20I%20tried%20submitting%20a%20project%20inquiry%20on%20the%20website%20and%20got%20an%20alert."
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-[#25D366] font-semibold hover:underline flex items-center gap-1 shrink-0"
                          >
