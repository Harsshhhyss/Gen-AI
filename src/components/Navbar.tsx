...
export const Navbar: React.FC = () => {
  ...
  return (
    <>
      <header className="fixed top-0 left-0 w-full p-4 sm:p-6 md:px-8 lg:px-12 md:py-6 z-[90] pointer-events-none">
        <div className="w-full flex justify-between items-center pointer-events-auto">
          ...
          <nav 
            className="hidden md:flex items-center gap-1 rounded-full px-3 py-1.5 backdrop-blur-md"
            style={{
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  to={link.path}
                  className={`relative px-3.5 lg:px-4 py-1.5 rounded-full text-xs lg:text-sm font-medium tracking-wider transition-all duration-300 group ${
                    active 
